...
sspemdd_sequential::sspemdd_sequential()
	: object_function_type("weighted"), h(0), H(0), ncb(1), nrhob(1), nR(41), ntau(1), cb1(2000.0),
	  ...
	  n_layers_w(1), iterated_local_search_runs(10), verbosity(1), N_total(1), rank(0)
{
...
-- experimental_delays: experimental_delays[ii][jj] is the delay of jj+1-th mode for the frequency
freqs[ii]

The routine computes the (uniform) residual (misfit) of experimental data and the "theoretical"
delays for a given environment model.

It should be used as follows: for a set of environment models the residual should be computed. The
minimal value of the residual indicates
	...
	std::vector<unsigned> &experimental_mode_numbers)
{
	...
	{
		// 2016.04.27:Pavel: mnumb = std::min(mode_numbers.at(ii),
		// experimental_mode_numbers.at(ii));
...
// 2016.12.31:Pavel: a residual functions where the "experimental" spectrogram modulud is taken as
	...
	for (unsigned ii = 0; ii < freqs.size(); ii++)
	{
		...
		for (unsigned jj = 0; jj < mnumb; jj++)
		{
			if (experimental_delays[ii][jj] > 0)
			{
				...
				residual =
					residual +
					weight_coeffs[ii][jj] * pow(experimental_delays[ii][jj] + tau - mdelay, 2);
			}
		}
	}
	// 2016.04.27:Pavel: RMS
...
int sspemdd_sequential::compute_modal_grop_velocities(
	std::vector<double> &freqs, double deltaf, std::vector<double> &depths,
	std::vector<double> &c1s, std::vector<double> &c2s, std::vector<double> &rhos,
	std::vector<unsigned> &Ns_points, unsigned flOnlyTrapped, unsigned &ordRich,
	std::vector<std::vector<double>> &modal_group_velocities, std::vector<unsigned> &mode_numbers)
...
1) It is better to choose mesh in such a way that mesh size is the same for all z. This gives better
accuracy!
(the reason: if the diagonal elements in the upper and lower diagonals vary with row index j, then
accuracy is crippled, the
...
*/
std::vector<double>
sspemdd_sequential::compute_wnumbers_extrap(double &omeg, // sound frequency
...
flOnlyTrapped -- flag to determine the mode subset: set to 0 to compute all propagating modes, i.e.
such that k^2>=0, otherwise only trapped modes are computed
...
std::vector<double> sspemdd_sequential::compute_wnumbers_extrap_lin_dz(
	double &omeg, // sound frequency
	std::vector<double> &depths, std::vector<double> &c1s, std::vector<double> &c2s,
	std::vector<double> &rhos, std::vector<unsigned> &Ns_points, unsigned flOnlyTrapped,
	unsigned &ordRich)
/*  subroutine for computing wavenumbers for a given waveguide structure
the computation is performed by the FD method for certain meshsize,
Richardson extrapolation is used to improve the
...
*/
{
	...
	case 2:
		...
		break;
	case 4:
		// coeff_extrap.assign({1.595325630252102, -0.788449059052564, 0.216346153846154,
		// -0.023222725045691});
...
std::vector<double> sspemdd_sequential::compute_wnumbers(
	double &omeg, // sound frequency
	std::vector<double> &c, std::vector<double> &rho, std::vector<unsigned> &interface_idcs,
	std::vector<double> &meshsizes,
	unsigned flOnlyTrapped // set flOnlyTrapped = 0 to compute all propagating modes, i.e. such that
						   // k^2>=0
	)
{
	...
	// for the k-th interface interface_idcs.at(k-1) we have meshsizes meshsizes.at(k-1) and
	// meshsizes.at(k) below and above the interface respectively
	...
	//        myFile << std::fixed << std::setprecision(16) << ld.at(ii) << "  " << md.at(ii) << "
	//        " << ud.at(ii) << endl;
	//    }
	//    myFile.close();
	double q = 0;
	...
	for (int ii = 0; ii < N_points - 2; ii++)
	{
		...
		{									 // ii -- z(ii+1), z(0) = 0
			...
			ld.at(ii) = 2 * q * dp / dz;
			md.at(ii) = -2 * q * (dz_next * dp + dz * dm) / (dz * dz_next) +
	...
	//    for (int ii=0; ii<N_points-3; ii++){
	//        myFile1 << std::fixed << std::setprecision(16) << main_diag[ii] << "  " <<
	//        second_diag[ii] << endl;
	//    }
	//    myFile1.close();

	alglib::smatrixtdevdr(main_diag, second_diag, N_points - 2, 0, kappamin * kappamin,
						  kappamax * kappamax, eigen_count, eigenvectors);
...
double sspemdd_sequential::fill_data_compute_residual(Point &point)
{   // finally specify sound speed in water
	...
	//	std::cout << "Layer #" << jj + 1 << ": c=" << c1s.at(jj) << "..." << c2s.at(jj) << "; rho="
	//<< rhos.at(jj) << "; np=" << Ns_points.at(jj) << std::endl;
	...
	{
		point.residual = compute_modal_delays_residual_weighted(
			freqs, depths, c1s, c2s, rhos, Ns_points, point.R, point.tau, modal_delays,
			weight_coeffs, mode_numbers);
	}
	else
	{
	...
	if (point.residual < record_point.residual)
	{
		...
		if (verbosity > 0)
		{
			...
			std::cout << "err = " << record_point.residual << ", parameters:" << std::endl;
			std::cout << "c_b = " << record_point.cb << ", rho_b = " << record_point.rhob
					  << ", tau = " << record_point.tau << ", R = " << record_point.R << std::endl;
...
void sspemdd_sequential::loadValuesToSearchSpaceVariables()
{
	...
	for (unsigned long long i = 0; i < cw1_arr.size(); i++)
	{
		...
		for (unsigned long long j = 0; j < ncpl_arr[i]; j++)
			tmp_vec[j] = cw1_arr[i] +
						 (ncpl_arr[i] == 1 ? 0 : j * (cw2_arr[i] - cw1_arr[i]) / (ncpl_arr[i] - 1));
		search_space.push_back(tmp_vec);
	}
...
void sspemdd_sequential::findLocalMinHillClimbing()
{
	...
	Point cur_point, global_record_point;
	...
	for (unsigned run_index = 0; run_index < iterated_local_search_runs; run_index++)
	{
		...
		do
		{					   // do while local min not reached
			isLocalMin = true; // if changing of every variable will not lead to a record updata,
							   // then a local min reached
			for (unsigned i = 0; i < search_space.size(); i++)
			{ // i stands for variable_index
				...
				do
				{ // change value of a variabble while it leads to updating of a record
					...
					{
						std::cout << "cur_point_indexes[i] == index_from. Break iteration."
								  << std::endl;
						break;
					}
					if (verbosity > 0)
					{
						std::cout << "checking index " << cur_point_indexes[i] << ", max index "
								  << search_space[i].size() - 1 << std::endl;
					}
					cur_point = fromPointIndexesToPoint(cur_point_indexes);
		...
		// std::cout << "global_record_point.residual " << global_record_point.residual <<
		// std::endl;
...
Point sspemdd_sequential::fromPointIndexesToPoint(std::vector<unsigned> cur_point_indexes)
{
	Point point;
...
Point sspemdd_sequential::fromDoubleVecToPoint(std::vector<double> double_vec)
{
	Point point;
...
double sspemdd_sequential::getRecordResidual() { return record_point.residual; }

void sspemdd_sequential::getThreeValuesFromStr(std::string str, double &val1, double &val2,
											   double &val3)
{
	val1 = val3 = -1;
	val2 = 1;
...
void sspemdd_sequential::readScenario(const std::string &scenarioFileName)
{
	/*
	...
	*/
	...
	{
		std::cerr << "scenarioFile with the name " << scenarioFileName << " wasn't openend"
				  << std::endl;
		exit(1);
	}
...
void sspemdd_sequential::readInputDataFromFiles()
{
	...
	{
		myLine.erase(std::remove(myLine.begin(), myLine.end(), '\r'),
					 myLine.end()); // delete windows endline symbol for correct reading
		myLineStream << myLine;
		myLineStream >> buff;
