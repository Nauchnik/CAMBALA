		...
		const std::vector<std::vector<double>> &modal_delays)
{
	...
	m_CopyToGPU2(R,   sz, ftype);
	m_CopyToGPU2(tau, sz, ftype);
	m_CopyToGPU2(rhob,sz, ftype);
	m_CopyToGPU2(cb,  sz, ftype);
	m_CopyToGPU2(cws, sz*cws_sz, ftype);
	m_CopyToGPU2(freqs, freqs_sz, ftype);
	m_CopyToGPU2(exp_delays, dmaxsz*freqs_sz, ftype);
	m_CopyToGPU2(exp_delays_sz, freqs_sz, int);
	m_CopyToGPU2(depths, n_layers, ftype);
	m_CopyToGPU2(Ns_points, n_layers, int);
	m_CopyToGPU2(residuals, sz, ftype);
	...
	checkCudaErrors(cudaMemcpy((void*) residuals, (void*)g_residuals, 
				sz*sizeof(ftype), cudaMemcpyDeviceToHost));
	for (size_t i = 0; i < sz; ++i)
