...
__device__ inline ftype sign_f( const ftype& val) { return (val < 0.0f) ? -1.0f : 1.0f; }

__device__ inline void StoreEv (ftype eiv, ftype* eigenvalues, int& eigenvalues_c) { eigenvalues[eigenvalues_c++] = eiv; }
...
__device__ int computeSmallerEigenvals (const ftype x, const ftype* md, const ftype* sd, const int mat_size)
{
	//assert (sd[0]==0.0f);
...
__device__ ftype midpoint(const ftype& left, const ftype& right)
{
...
__device__ inline ftype midpoint(const Interval& curint) { return midpoint (curint.ll, curint.rl);};
...
__device__ int bisectGPU (const ftype* md, const ftype* sd, const int mat_size,
